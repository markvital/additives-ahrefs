...
CLI flags (`--additive`, `--override`, `--debug`, etc.). The script expects an existing production build in `.next/`—run
`npm run build` first (or supply `--base-url` to reuse a running server) when invoking it manually. Playwright bundles its
Chromium binary during `npm install`, so no extra setup is required unless the generator reports that the browser is missing,
in which case run `npx playwright install chromium` once. Set `CARD_PREVIEW_SKIP=1` to bypass the automated step when iterating
locally.

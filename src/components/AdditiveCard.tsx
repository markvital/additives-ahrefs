...
export interface AdditiveCardProps {
...
export function AdditiveCard({
  ...
  const hasSparkline = Array.isArray(item.searchSparkline) && item.searchSparkline.some((value) => value !== null);
  const hasSearchMetrics = typeof item.searchRank === 'number' && typeof item.searchVolume === 'number';
  const showSearchSection = hasSparkline || hasSearchMetrics;
  const origins = item.origin.filter((origin) => origin.trim().length > 0);
  const highlightProducts = sortMode === 'product-count';
  const searchSectionOpacity = highlightProducts ? 0.6 : 1;
  const productCountValue = typeof item.productCount === 'number' ? Math.max(0, item.productCount) : null;
  ...
  const effectiveAwarenessScore = awarenessScore ?? item.awarenessScore ?? null;
  ...
  return (
    <Card data-additive-card-index={typeof index === 'number' ? index : undefined} sx={{ display: 'flex', flexDirection: 'column', ...cardSx }}>
      <CardActionArea
        {...actionAreaProps}
        sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', alignItems: 'stretch', height: '100%' }}
        disableRipple={!linkHref}
      >
        <CardContent
          sx={{
            display: 'flex',
        ...
        >
          <Stack spacing={1.5} sx={{ flexGrow: 1 }}>
            ...
            {item.functions.length > 0 ? (
              <FunctionChipList functions={item.functions} sx={{ maxWidth: '100%' }} />
            ) : (
              <Box sx={{ minHeight: 28 }} />
            )}
          </Stack>
          ...
          {showSearchSection ? (
            ...
            >
              {hasSearchMetrics ? (
                <Stack direction="row" alignItems="baseline" spacing={1} flexShrink={0}>
                  ...
                  <Typography component="span" variant="body2" color="text.secondary">
                    {formatMonthlyVolume(item.searchVolume!)} / mo
                  </Typography>
                </Stack>
              ) : (
                <Box sx={{ minWidth: 0 }} />
              ...
              {hasSparkline ? (
                ...
                >
                  <SearchSparkline values={item.searchSparkline ?? []} />
                </Box>
              ) : (
          ...
          {showProductCount && productCountLabel ? (
            ...
            >
              ...
              >
                Found in <Box component="span" sx={{ fontWeight: 600, color: '#5c5c5c' }}>{productCountLabel}</Box>{' '}
                products
              </Typography>
              {effectiveAwarenessScore ? (
                <AwarenessScoreChip score={effectiveAwarenessScore} labelStyle="grid" sx={{ ml: 'auto', flexShrink: 0 }} />
              ) : null}
            </Box>
          ) : effectiveAwarenessScore ? (
            <Box sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end' }}>
              <AwarenessScoreChip score={effectiveAwarenessScore} labelStyle="grid" />
            </Box>
          ) : null}
        </CardContent>
      </CardActionArea>
...
export const additiveCardHelpers = {
  hyphenateLongWords,
